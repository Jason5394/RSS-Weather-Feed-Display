...
class WeatherController:
    ...
    def pressedChangeRss(self):
        ...
        if self.toplevels["changeRSS"] is None:
            ...
            self.toplevels["changeRSS"].submit_rss_button.config(
                command=self.pressedSubmit)
            self.toplevels["changeRSS"].rss_entry.bind(
                "<Return>", self.pressedReturnHandler)
    ...
    def pressedSubmit(self):
        ...
        # if okay, exit out of window and save new url otherwise show warning
        # message
