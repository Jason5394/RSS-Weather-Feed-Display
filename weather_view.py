...
class WeatherView(ttk.Frame):
    ...
    def __init__(self, root, *args, **kwargs):
        ...
        self.refresh_button = ttk.Button(
            self.topframe, text="Update", width=12)
        self.change_rss_button = ttk.Button(
            self.topframe, text="Change feed", width=12)
        self.show_src_button = ttk.Button(
            self.topframe, text="Show source", width=12)

        # create a label for each data member
        self.title_label = ttk.Label(
            self.topframe, text="Weather Forecast", font="bold")

        self.temperature_label = ttk.Label(self.midframe,
                                           textvariable=self.temperature, padding=5, font="Calibri, 24")
        self.conditions_img_label = ttk.Label(
            self.midframe, image=self.conditions_img)
        ...
        self.pressure_label = ttk.Label(
            self.botframe, textvariable=self.pressure)
        self.humidity_label = ttk.Label(
            self.botframe, textvariable=self.humidity)
        self.wind_label = ttk.Label(self.botframe, textvariable=self.wind)
        self.heat_index_label = ttk.Label(
            self.botframe, textvariable=self.heat_index)
        self.last_updated_label = ttk.Label(
            self.botframe, textvariable=self.last_updated)
        self.location_label = ttk.Label(
            self.botframe, textvariable=self.location)
        # static labels that don't change
        self.pressure_static_label = ttk.Label(
            self.botframe, text="Pressure: ")
        self.humidity_static_label = ttk.Label(
            self.botframe, text="Humidity: ")
        self.wind_static_label = ttk.Label(
            self.botframe, text="Wind conditions: ")
        self.heat_static_label = ttk.Label(self.botframe, text="Heat index: ")
        self.location_static_label = ttk.Label(
            self.botframe, text="Location: ")
        self.updated_static_label = ttk.Label(
            self.botframe, text="Last updated: ")
...
class ShowSourceWindow(FormTopLevel):
    ...
    def __init__(self, root, src="", **kwargs):
        ...
        ttk.Label(self.frame, text="RSS Source",
                  font="bold").grid(column=0, row=0)
...
class ChangeRSSWindow(FormTopLevel):
    ...
    def __init__(self, root, **kwargs):
        ...
        ttk.Label(self.frame, text="Enter the url of the RSS feed:").grid(
            column=0, row=0)
